...
const DirectionPage = () => {
  ...
  const { first_subject, second_subject } = useSelector(
    (state) => state.subjectsReducer
  );
  const token = JSON.parse(localStorage.getItem("token"));

  console.log(facultyInfo.get_count, facultyInfo);
  ...
  const deleteFaculty = (faculty_id) => {
    setFaculties((state) =>
      state.filter((faculty) => faculty.faculty_id !== faculty_id)
    );
  };
  ...
  useEffect(() => {
    ...
    fetch(
      `${HOST}/universities?first_subject=${first_subject}&second_subject=${second_subject}`,
      { headers: { token } }
    )
      .then((res) => res.json())
      .then((res) => {
  ...
  return (
    <div className={style.directionBlock}>
      <h1 className={style.directionBlock__title}>
        Assosiy Imtihonga hush kelibsiz
      </h1>
      ...
      <div className={style.directionBlock__container}>
        ...
        <span>
          <div
            className={
              faculties.length ? style.directionBlock__facultyBlokc : style.none
            }
          >
            {faculties.length > 0 &&
          ...
          <div
            className={
              facultyInfo.length
                ? style.directionBlock__facultyInfo
                : style.none
            }
          >
            <h2 className={style.directionBlock__facultyInfo__title}>
              {facultyInfo[0]?.faculty_name}
            </h2>
            <p className={style.directionBlock__facultyInfo__desc}>
              {facultyInfo[0]?.region_name}
            </p>
            <TableItem
              name={"Grant"}
              count={facultyInfo[0]?.grant_count}
              ball={facultyInfo[0]?.passing_score_grant}
            />
            <TableItem
              name={"Shartnoma"}
              count={facultyInfo[0]?.contract_count}
              ball={facultyInfo[0]?.passing_score_contract}
            />
          </div>
        </span>
      </div>
      <button onClick={sendInfo} className={style.directionBlock__btn}>
        Next
      </button>
    </div>
  );
